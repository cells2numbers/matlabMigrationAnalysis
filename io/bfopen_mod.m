function [numImages] = bfopen_mod(id,imInfos,varargin)
...
% TODO check optional input arguments
% imageCount etc
i=1;
while i<=length(varargin),
    ...
    if ischar(varargin{i}),
        switch varargin{i},
            case 'imageCount',    i=i+1; imageCount = varargin{i};
            case 'verbose',       i=i+1; verbose = varargin{i};
            case 'convert',       i=i+1; CONVERT = varargin{i};
            case 'pathFolder',    i=i+1; pathFolder = varargin{i};
            otherwise
                argok=0;
        end
    else
    ...
    if ~argok,
        disp(['bfopen_mod.m : WARNING! Ignoring invalid argument #' num2str(i+2)]);
    end
    i = i+1;
end
...
for s = 1:numSeries
    ...
    % in the original bfopen, all images are stored in results,
    % -> this will crash matlab on computers with too few memory
    %
    % 
    %
    %    imageList{i, 1} = arr;
    ...
    if CONVERT
        arr = convertImage(arr);
    end
    
    
    if isempty(pathFolder)
        saveImageName = getFilename2(imInfos,i + imageCount);
    else 
       saveImageName = [pathFolder filesep sprintf('image%04i.png',i + imageCount) ];
    end
    
    
    imwrite(arr,saveImageName,'png');
    end
